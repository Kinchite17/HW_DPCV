...
class VAE(nn.Module):
    ...
    def forward(self, x):
        ...
        x_hat = x_hat_flat.view(batch_size, 1, int((self.input_size)**0.5), int((self.input_size)**0.5))
        ###########################################################################
        #                                      END OF YOUR CODE                   #
        ###########################################################################
...
def loss_function(x_hat, x, mu, logvar):
    ...
    BCE = F.binary_cross_entropy(x_hat_flat, x_flat, reduction='sum') / batch_size
