...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
        ...
        diff = train_sample.unsqueeze(0) - x_test_flat  
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    dot_products = torch.mm(x_train_flat, x_test_flat.T)  
    dists = (train_sq_norms.unsqueeze(1) + 
             test_sq_norms.unsqueeze(0) - 
             2 * dot_products)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    for j in range(num_test):
        distances_to_test_j = dists[:, j]  
        _, closest_indices = torch.topk(distances_to_test_j, k, largest=False)
        closest_labels = y_train[closest_indices]  
        if k == 1:
            y_pred[j] = closest_labels[0]
        else:
            vote_counts = torch.bincount(closest_labels)
            y_pred[j] = torch.argmax(vote_counts)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return y_pred
...
def knn_cross_validate(
    ...
    for k in k_choices:
        ...
        for fold_idx in range(num_folds):
            ...
            x_train_cv = torch.cat(x_train_cv, dim=0)
            y_train_cv = torch.cat(y_train_cv, dim=0)
            knn_classifier = KnnClassifier(x_train_cv, y_train_cv)
            accuracy = knn_classifier.check_accuracy(x_val, y_val, k=k, quiet=True)
...
def knn_get_best_k(k_to_accuracies: Dict[int, List]):
    ...
    for k, accuracies in k_to_accuracies.items():
        ...
        if (mean_accuracy > best_mean_accuracy or 
            (mean_accuracy == best_mean_accuracy and k < best_k)):
