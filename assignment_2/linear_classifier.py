...
def svm_loss_vectorized(
    ...
    y = y.to(device=W.device)
    
    num_train = X.shape[0]
    scores = X.mm(W)  
    correct_class_scores = scores[torch.arange(num_train, device=scores.device), y].unsqueeze(1)  

    margins = scores - correct_class_scores + 1  

    margins[torch.arange(num_train, device=margins.device), y] = 0
    loss = torch.sum(torch.clamp(margins, min=0)) / num_train
    loss += reg * torch.sum(W * W)
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
    ...
    #############################################################################
    # Replace "pass" statement with your code
    binary = (margins > 0).to(dtype=X.dtype)  
    row_sum = torch.sum(binary, dim=1)  
    binary[torch.arange(num_train, device=binary.device), y] = -row_sum
    dW = X.t().mm(binary) 
    dW /= num_train
    dW += 2 * reg * W
    #############################################################################
...
def svm_get_search_params():
    ...
    regularization_strengths = [1e-6, 1e-5, 5e-5, 1e-4, 5e-4]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
...
def softmax_loss_vectorized(
    ...
    probs = exp_scores / sum_exp_scores 
    correct_class_probs = probs[torch.arange(num_train, device=probs.device), y]  
    loss = torch.sum(-torch.log(correct_class_probs)) / num_train
    y_onehot = torch.zeros_like(probs) 
    y_onehot[torch.arange(num_train, device=y_onehot.device), y] = 1.0
    dscores = (probs - y_onehot) / num_train 
    dW = X.t().mm(dscores) 
    loss += reg * torch.sum(W * W)
    dW += 2 * reg * W
    #############################################################################
