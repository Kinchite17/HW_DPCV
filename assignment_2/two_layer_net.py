...
def nn_forward_pass(params: Dict[str, torch.Tensor], X: torch.Tensor):
    ...
    hidden = torch.relu(hidden_pre_relu)  
    scores = hidden @ W2 + b2  
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def nn_forward_backward(
    ...
    data_loss = torch.mean(correct_log_probs)
...
def nn_get_search_params():
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    learning_rates = [  5e-1, 6e-1, 7e-1, 8e-1, 9e-1, 1.0, 2.0]
    hidden_sizes = [ 512, 1024, 2048]
    regularization_strengths = [1e-5, 5e-5, 1e-4, 5e-4]
    learning_rate_decays = [0.86 ,0.89, 0.92, 0.95]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
...
def find_best_net(
    ...
    total_combinations = len(learning_rates) * len(hidden_sizes) * len(regularization_strengths) * len(learning_rate_decays)
    combination_count = 0
        ...
        for hidden_size in hidden_sizes:
            for reg in regularization_strengths:
                for lr_decay in learning_rate_decays:
                    ...
                    #      f"lr={lr:.1e}, hidden={hidden_size}, reg={reg:.1e}, decay={lr_decay:.2f} "
